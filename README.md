...
- **対戦開始フロー & 難易度選択**:
  - タイトル画面で対局モード（1P黒先手 / 1P白後手 / CPU観戦）を選択後、即座に**難易度選択（初級 EASY / 中級 NORMAL / 上級 HARD）**モーダルが表示され、好みの強さを選んで対局開始可能。
- **ハイブリッド操作システム**:
  - **コントローラー操作**: 盤面上を十字キー & 左スティックで選択カーソル移動、Aボタンで着手、BボタンでUndo、Xボタンでガイド切替、Startボタンで設定メニュー。
